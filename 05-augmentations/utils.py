...
def show_single_augmentation(original_img, augmented_img, title="Аугментация"):
    ...
    # Оригинальное изображение (PIL.Image -> Tensor)
    orig_tensor = to_tensor(original_img)
    orig_resized = transforms.Resize((128, 128), antialias=True)(orig_tensor)
    orig_np = orig_resized.numpy().transpose(1, 2, 0)
    orig_np = np.clip(orig_np, 0, 1)
    
    # Аугментированное изображение (уже тензор)
    aug_resized = transforms.Resize((128, 128), antialias=True)(augmented_img)
...
def show_multiple_augmentations(original_img, augmented_imgs, titles):
    ...
    # Преобразуем оригинальное изображение в тензор
    to_tensor = transforms.ToTensor()
    resize = transforms.Resize((128, 128), antialias=True)
    
    # Оригинальное изображение (PIL.Image -> Tensor -> Resize)
    orig_tensor = to_tensor(original_img)
    orig_resized = resize(orig_tensor)
    orig_np = orig_resized.numpy().transpose(1, 2, 0)
    orig_np = np.clip(orig_np, 0, 1)
    ...
    # Аугментированные изображения (уже тензоры)
    for i, (aug_img, title) in enumerate(zip(augmented_imgs, titles)):
        aug_resized = resize(aug_img)
        aug_np = aug_resized.numpy().transpose(1, 2, 0)
        aug_np = np.clip(aug_np, 0, 1)
